...
// ── 최근 본 글 ───────────────────────────────────────────────
//   ★★ week15가 "못 만든다"고 비워뒀던 자리다. 세션이 생겨서 되살아났다.
//
//   [왜 week15에서는 못 만들었나]
//     '최근 본 글'은 "이 브라우저가 방금 뭘 봤는지"를 **계속 쌓아야** 하는 기능이다.
//     week15는 서버가 기억할 것을 전부 주소에 실어 날랐는데, 이건 실을 수가 없었다 —
//     ?recent=1,5,9,12,… 가 볼수록 길어져서 주소가 감당하지 못한다.
//     '한 번 쓰고 버리는 값'(신원·알림)은 주소로도 되지만, '쌓이는 값'은 안 된다.
//     → 이 빈자리가 곧 **세션이 왜 필요한가**에 대한 답이었다.
//
//   [왜 DB 표(recent_views)로 안 만드나]
//     표를 파면 기록의 주인이 '브라우저'에서 '회원'으로 바뀐다.
//     → 로그인 안 한 방문자는 아예 기록되지 않는다. 같은 기능이 아니라 **다른 기능**이 된다.
//     "이 브라우저가 방금 본 것"은 세션이 딱 맞는 그릇이다. (쇼핑몰 '최근 본 상품'과 같다)

// 세션에 목록을 넣을 때 쓰는 열쇠 이름. (SESSION_USER_ID·SESSION_FLASH와 같은 이유로 상수)
const SESSION_RECENT_POSTS = 'recent_posts';

// 몇 개까지 기억할지. 홈 사이드바 한 칸에 들어갈 만큼만.
const RECENT_POSTS_MAX = 5;

// 방금 본 글을 목록 맨 앞에 넣는다. (post/view.php가 호출)
//
//   ★ "GET 화면인데 세션에 뭘 쓰는 게 규칙 위반 아닌가?" — 아니다.
//     GET이 지켜야 할 '안전(safe)' 규칙은 **남들이 보는 자료를 바꾸지 말라**는 뜻이다.
//     여기서 남기는 건 '내 열람 기록'일 뿐, 글·댓글은 하나도 안 건드린다.
//     그래서 새로고침해도, 남이 같은 주소를 열어도 남들에게 보이는 결과는 달라지지 않는다.
//     (쇼핑몰의 '최근 본 상품'도, 조회수 집계도 같은 이유로 GET에서 처리한다)
function remember_recent_post(int $id): void {
    $recent = $_SESSION[SESSION_RECENT_POSTS] ?? [];

    // 이미 목록에 있으면 일단 빼낸다 → 같은 글이 여러 줄로 쌓이는 걸 막고,
    // 아래에서 맨 앞에 다시 넣으면 자연스럽게 '가장 최근'으로 올라온다.
    //   array_values = 걸러내며 생긴 번호 구멍(0,2,3…)을 0,1,2…로 다시 매긴다.
    $recent = array_values(array_filter($recent, fn($seenId) => $seenId !== $id));

    array_unshift($recent, $id);          // 맨 앞에 넣기 (배열의 '앞으로 밀어넣기')

    // 넘치면 오래된 것부터 버린다. 앞이 최신이므로 앞에서 N개만 남기면 된다.
    $_SESSION[SESSION_RECENT_POSTS] = array_slice($recent, 0, RECENT_POSTS_MAX);
}

// 최근 본 글 목록 (화면이 바로 쓸 수 있는 글 배열로 돌려준다)
//   ★ 그 사이 지워진 글은 get_post()가 null을 주므로 자동으로 빠진다 (Tester-Doer).
//     세션에 남은 번호를 따로 청소할 필요가 없다 — 화면에 안 나오면 그만이다.
//   ★ 하나씩 조회하는 이유: 최대 5개뿐이라 비용이 작고, get_post()를 그대로 써서
//     다른 목록들과 **똑같은 모양의 배열**이 나온다 → 홈 화면 코드를 그대로 재사용한다.
//     (개수가 커진다면 WHERE id IN (…) 으로 한 번에 가져와야 한다)
function get_recent_posts(): array {
    $result = [];
    foreach ($_SESSION[SESSION_RECENT_POSTS] ?? [] as $id) {
...
//     쿠키에 두면 값이 사용자 PC에 있다 → 지우고 새로고침, 지우고 새로고침을 반복해
//     **조회수를 얼마든지 부풀릴 수 있다.** 조회수는 랭킹·정렬에 쓰이므로
//     '틀리면 손해 보는 값'이다 → 서버가 들고 있어야 한다. 그래서 세션이다.
//     (반대로 '게시판 정렬 기본값' 같은 건 틀려도 취향일 뿐이라 쿠키로 간다)
//
//   ※ 세션이 살아있는 동안만 기억하므로, 브라우저를 새로 켜면 같은 글이 한 번 더 세어진다.
//     완벽하지는 않지만 '새로고침 연타'라는 제일 흔한 부풀리기를 막는다.
//     (완전히 막으려면 회원별로 DB에 남겨야 하는데, 그러면 비회원 조회가 안 세어진다)

// 이번 방문에 이미 본 글 번호들을 담는 세션 열쇠.
const SESSION_VIEWED_POSTS = 'viewed_posts';

// 이 글의 조회수를 1 올린다. 단, 이번 방문에 이미 봤으면 아무 일도 하지 않는다.
//   돌려주는 값 = 실제로 올렸는지. 화면이 이 값을 보고 방금 올린 1을 반영한다
//   (그러려고 글을 DB에서 한 번 더 읽는 건 낭비라서).
function count_post_view(int $id): bool {
    // in_array($needle, $haystack, true) — 세 번째 true는 '타입까지 같아야 한다'.
    //   안 주면 '1'(글자)과 1(숫자)을 같다고 봐서 엉뚱한 글이 이미 본 것으로 처리될 수 있다.
    $viewed = $_SESSION[SESSION_VIEWED_POSTS] ?? [];
    ...
    $viewed[] = $id;
    $_SESSION[SESSION_VIEWED_POSTS] = $viewed;
    return true;
}
