...
**이동 선택** (AiInput): 합법적인 수 중 랜덤으로 선택

**스킬/아이템 선택** (SkillAiPlayer): 확률 기반 행동 선택
1. 파괴 스킬: 폰 제외 상대 기물이 있을 때 33% 확률로 사용 (가장 가치 높은 기물 대상)
2. 부활 스킬: 가치 5 이상인 잡힌 기물이 있으면 사용
3. 방패 스킬: 20% 확률로 사용
4. 아이템 설치: 30% 확률로 중앙 근처 빈 칸에 설치
5. 일반 이동
