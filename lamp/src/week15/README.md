...
**세션은 하나도 쓰지 않는다.** 서버가 기억해야 할 것은 전부 주소에 실어 나른다 —
신원은 `?as=아이디`, 알림은 `?flash=…&ftype=…`. 나머지 데이터는 전부 DB.

- 링크마다 손으로 붙이지 않는다. PHP 내장 URL 리라이터(`output_add_rewrite_var`)가
  출력되는 모든 링크에 자동으로 붙이고, 폼에는 hidden 필드를 심어준다 (`includes/header.php`).
  JS가 만드는 링크만은 리라이터가 못 건드리므로 `main.js`의 `withIdentity()`를 통과시킨다.
- **'최근 본 글'은 week16으로 미뤘다** — 계속 쌓이는 목록이라 주소로는 못 나른다
  (`?recent=1,5,9,…`가 무한정 길어진다). 서버가 브라우저별로 뭔가를 계속 들고 있으려면
  세션이 필요하다. 이 빈자리가 곧 **세션이 왜 있어야 하는지**에 대한 답이다.
