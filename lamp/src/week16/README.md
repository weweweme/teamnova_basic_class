...
남의 사이트가 우리 폼을 흉내 내 POST를 쏘는 공격. 막으려면 서버가 "이 토큰, 내가 발급한 게
맞나"를 대조해야 하는데, week15는 **발급한 사실 자체를 기억할 곳이 없어서** 시작조차 못 했다.
→ 세션이 생겼으므로 이제 가능하다. **11절 ③**에서 만든다.
(`SameSite=Lax`가 이미 상당 부분을 막지만, 브라우저 설정에 기대는 방어라 토큰이 정석이다)
...
#### ③ CSRF 토큰 (새 보안 — 세션이 있어야 비로소 가능)

남의 사이트가 우리 폼을 흉내 내 POST를 쏘는 공격. 막으려면 서버가
"이 토큰, 내가 발급한 게 맞나"를 대조해야 하는데 — **week15는 발급한 사실을 기억할 곳이 없었다.**

- 세션에 랜덤 토큰을 두고, 폼에 hidden으로 심고, 액션 파일에서 `hash_equals()`로 대조.
  (`===` 대신 `hash_equals()`인 이유: 비교에 걸리는 **시간 차이**로 토큰을 한 글자씩
  알아내는 공격이 있다. 이 함수는 항상 같은 시간이 걸리게 비교한다)
- `csrf_field()` / `require_csrf()` **두 함수**로 만들고, POST 액션 파일 전부에 적용한다.
