<?php
// ============================================================
// session/refresh.php — 도장을 다시 확인받는다  [POST → JSON]
//
//   [★ '세션을 연장한다'가 아니다 — 정확히 말하면]
//     여기서 갱신되는 건 **'마지막으로 도장을 확인한 시각'** 하나뿐이다.
//     세션 수명(TTL 30분·유휴 20분)은 건드리지 않는다.
//     확인이 만료되면 세션도 로그인 상태도 **그대로 살아 있지만**,
//     그 세션으로는 **아무 화면도 못 연다** — 전부 '기기 확인'으로 막힌다.
//
//   [★ 이 파일이 이번 변경의 전부다]
//     전:  요청만 오면        계속 쓸 수 있다   ← 훔친 쪽도 요청만 보내면 무기한
//     후:  도장을 확인받아야  계속 쓸 수 있다   ← 훔친 쪽은 도장이 없어 1분 뒤 막힌다
//
//   [★★ 그래서 훔친 쿠키의 수명이 '1분'으로 못 박힌다]
