...
- `week16/` : **지금 여기**. week15를 통째로 복사해서 시작 (2026-08-03).
  ...
  - **①~⑪ 전부 완료.** 정리: 신원·플래시·CSRF·최근 본 글·조회수·**저장소**·**로그인 복귀**·**입력값** → **세션**,
    자동 로그인·정렬/검색 취향·**다른 기기 로그아웃** → **쿠키**(+DB).
  - **DB 표 10개**가 됐다: week15의 8개 + `remember_tokens`(자동 로그인) + `sessions`(세션 저장소).
    마이그레이션은 `005_sessions.sql`까지 적용됨 (001~003은 week15, 004~005는 week16).
