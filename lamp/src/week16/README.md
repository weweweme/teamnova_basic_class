...
- **알림(flash)은 아직 주소로 나른다** — `?flash=…&ftype=…`. 11절 ②에서 세션으로 옮긴다.
...
지금은 `?flash=…&ftype=…&fundo=…&fid=…` **파라미터 4개**를 주소로 나른다.
그 탓에 **우회책이 둘** 붙어 있다 — ⓐ JS로 주소창 청소, ⓑ `query_url()`에서 `FLASH_KEYS` 제외.

→ 세션에 넣고 **읽으면서 지운다**(read-once). 파라미터 4개와 우회책 2개가 **전부 사라진다**.
Rails·Laravel이 쓰는 실무 표준 방식이고, week15 주석이 "예전 방식"이라 부르던 바로 그것이다.
