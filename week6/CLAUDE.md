...
**조작**: 터미널 raw 모드 (화살표 키 즉시 입력, 한글 IME 자동 변환)
- ↑↓: 정착민 선택
- 1~6: 수리/무기/치료/건설/모집/승격
- n: 밤으로 건너뛰기
- 0: 치트 모드 (1보급 2회복 3전멸 4무적 5바리MAX 6미니건 7적강화 8적약화 9정착민즉사)
- q: 타이틀로/취소 (한글 ㅂ도 동일)
- 배치 모드 (건설→구조물 선택 후): ↑↓←→ 커서 이동, Enter 설치, q 취소
- 타이틀: 1/2/3 난이도 선택, q 프로그램 종료
...
- 정착민: 사망 시 회색 → 페이드 아웃 (800ms), 사망 후 패널 목록에서 숨김
- 총알/이펙트: 무기별 색상
- 치명타: 밝은 빨강 `*` 3칸 (상/중/하)
