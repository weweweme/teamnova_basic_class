...
- [ ] **DBeaver**: `review_community` 접속 → 아래 표들 **Data 탭 미리 열기**
  ...
  - 새로고침 = **⟳ 버튼** 또는 `Ctrl+R`
- [ ] ★ **`likes` 탭에는 필터를 걸어둔다** → 필터 칸에 `post_id = 120`
  - 이유: `likes`는 `user_id`·`post_id` **두 칼럼뿐**이라 `id`가 없다.
    다른 표에 쓰는 "id 내림차순 → 새 행이 맨 위" 전략이 **이 표에서만 통하지 않는다.**
    1,440행 중 어디에 생겼는지 눈으로 못 찾으므로, 필터로 **8행짜리 표**로 만들어 둔다.
  - 120번 글(`기대 이상이었습니다`) = 추천 8개, **영화광은 안 누른 상태** — seed 기준이라
    다른 기기에서도·초기화 후에도 똑같다.
  - ※ 시연 전에 한 번 확인: `SELECT COUNT(*) FROM likes WHERE post_id=120;` → **8**
    (개발하며 클릭한 게 쌓여 숫자가 다를 수 있다. 다르면 아래 대본의 숫자만 맞춰 읽으면 된다)
- [ ] **SQL 에디터 탭 2개를 미리 열어둔다** (장면 4에서 바로 실행)
  ```sql
  -- ① 추천한 사람 '이름' 목록
  SELECT u.nickname AS 추천한사람
  FROM likes l JOIN users u ON u.id = l.user_id
  WHERE l.post_id = 120;

  -- ② 복합키 증명용 (이미 있는 조합을 일부러 다시 넣어본다)
  INSERT INTO likes (user_id, post_id) VALUES (3, 120);
  ```
...
#### ★ 예상 질문 2: "신원은 주소에 있는데, 비밀번호는 왜 주소에 없나요?"

비밀번호는 **로그인할 때 딱 한 번, 폼의 몸통(POST)으로만** 간다. 그 뒤로는 **다시 보내지 않는다.**

```
[로그인 1회]  POST /auth/authenticate.php    몸통: username=영화광&password=1234
[그 뒤 전부]  GET  /board/?as=영화광          ← 비밀번호 없음. 아이디만.
```

`auth/login.php`의 폼이 `method="post"`이고, 코드 주석에도 이렇게 적혀 있다:
*"로그인 정보는 민감하므로 반드시 POST (주소에 비밀번호가 남으면 큰일)"*

**왜 '주소에 남으면 큰일'인가 — 주소는 사방에 복사되기 때문**

| 남는 곳 | 설명 |
|---|---|
| 브라우저 방문기록·자동완성 | 다음에 쓰는 사람이 주소창만 눌러도 보인다 |
| 서버 접근로그 | Apache가 요청 URL을 **통째로** 기록한다 |
| **Referer 헤더** | 다른 페이지로 넘어갈 때 "어디서 왔는지"가 함께 전송된다. **같은 사이트 안에서는 쿼리까지 전부** 간다 |
| 화면 그 자체 | 주소창에 그대로 보인다 — **지금 이 발표처럼 화면을 공유하는 순간 전원에게 공개** |

> Referer는 한 가지만 짚고 넘어갈 것: 요즘 브라우저는 **외부 사이트로 나갈 땐** 도메인만 보내도록
> 기본값이 바뀌었다(`strict-origin-when-cross-origin`). 그래서 우리 페이지의 TMDB 포스터를 받아올 때
> `image.tmdb.org`에 전체 주소가 넘어가지는 **않는다.** 하지만 그건 **브라우저의 기본 정책일 뿐
> 서버가 보장하는 게 아니고**, 같은 사이트 안 이동에서는 여전히 전체 주소가 간다.
> "브라우저가 알아서 막아주니 괜찮다"는 근거로 삼을 수 없다는 뜻.

마지막 줄이 제일 실감난다. **지금 주소창을 띄워놓고 발표하는 중인데**, 비밀번호가 거기 있었다면 이 자리에서 유출된 것이다.

**"그럼 `?as=영화광`은 왜 주소에 있어도 되나요?"**
→ **비밀이 아니기 때문.** 아이디는 프로필 주소(`/profile/?user=영화광`)에도 이미 드러나는 공개 정보다.
남이 봐도 잃을 게 없다. `as`의 문제는 **유출이 아니라 위조** — 성격이 다른 문제다.

**"그럼 비밀번호를 계속 주소에 실으면 위조도 막히지 않나요?"**
→ 막히긴 한다. 대신 위 표의 **모든 경로로 비밀번호가 새어나간다.** 위조 하나 막자고 비밀 자체를 뿌리는 꼴이다.
그래서 실무는 비밀번호 대신 **'증표'** 를 주고받는다:

| 증표가 갖춰야 할 것 | `?as=영화광` | week16의 세션 ID |
|---|---|---|
| ① 서버가 발급한다 | ✗ 사용자가 직접 적는다 | ✓ 로그인 성공 시 서버가 준다 |
| ② 추측할 수 없다 | ✗ 아이디만 알면 끝 | ✓ 길고 무작위인 문자열 |
| ③ 언제든 무효화할 수 있다 | ✗ 방법이 없다 | ✓ 서버가 지우면 즉시 끝 |

> **한 줄**: 비밀번호는 **한 번만, 몸통으로.** 그 뒤엔 비밀번호가 아니라 **증표**를 들고 다닌다.
> 지금 우리 증표는 누구나 베껴 쓸 수 있는 **이름표**라서 위조되고,
> week16의 증표는 **추측할 수 없는 번호표**라서 위조되지 않는다.

**⚠️ 정확히 말하면** — POST가 GET보다 '암호화돼서' 안전한 게 아니다.
둘 다 평문이라 **네트워크를 엿보면 똑같이 보인다.** 차이는 오직 **기록으로 남느냐**다.
도청까지 막는 건 **HTTPS(TLS)** 이고, 실무 로그인은 예외 없이 HTTPS 위에서 한다.
(우리는 `localhost` 개발 환경이라 http를 쓴다 — 이것도 물어보면 이렇게 답한다)
...
### 5. 관계 — 복합키·투표·신고 ⭐
**추천 (likes 토글)** — 120번 글(`기대 이상이었습니다`)로 시연. `likes` 탭에 `post_id = 120` 필터 필수.

*(1) 행이 생겼다 사라지는 걸 보여준다*
- **[DBeaver]** `likes` ⟳ → **8행** (필터 덕분에 한눈에 셀 수 있다)
- **[홈페이지]** 영화광으로 👍 추천 → ⟳ → **9행** (영화광 줄이 생김)
- **[홈페이지]** 다시 클릭(취소) → ⟳ → **8행** (그 줄이 사라짐)
- **[말]** *"추천은 `id` 없이 `(user_id, post_id)` 짝 자체가 기본키인 표입니다. 취소하면 표식이 남는 게 아니라 **줄이 통째로 사라집니다.**"*
  - ※ 글·댓글의 소프트삭제(`deleted_at`에 표식만)와 **정반대**라는 점을 짚으면 3번 구간과 잘 이어진다.

*(2) 숫자 대신 '이름'으로 — 미리 열어둔 SQL 에디터 ①*
```sql
SELECT u.nickname AS 추천한사람
FROM likes l JOIN users u ON u.id = l.user_id
WHERE l.post_id = 120;
```
- 심야극장 · 자막수집가4 · 명장면7 … 목록에 **영화광이 나타났다 사라진다**
- **[말]** *"저장은 번호로, 화면은 JOIN으로 이름. 2번에서 본 그 구조가 여기서도 똑같이 쓰입니다."*

*(3) ★ 킬러 — 복합키를 직접 때려본다 (SQL 에디터 ②)*
```sql
INSERT INTO likes (user_id, post_id) VALUES (3, 120);   -- 심야극장은 이미 추천함
```
```
ERROR 1062 (23000): Duplicate entry '3-120' for key 'PRIMARY'
```
- **[말]** *"제가 지금 **PHP 코드를 완전히 우회해서** DB에 직접 넣으려 했는데도 막혔습니다.
  1인 1회는 코드가 지키는 게 아니라 **DB 구조가 지킵니다.**"*
- ※ 실패하는 쿼리라 데이터는 변하지 않는다. 몇 번을 눌러도 안전하다.
...
| 4 | 추천/취소 | likes (**`post_id=120` 필터**) | 8행↔9행 · 닉네임 목록 · 중복 INSERT 거부 |
...
3. **복합키가 '1인 1회'를 강제** — 코드를 우회한 직접 INSERT도 DB가 거부 (장면 4·5)
