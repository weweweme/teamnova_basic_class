...
**아직 못 막는 것 두 가지 — 둘 다 "서버가 아무것도 기억하지 않아서" 생긴다:**

| 구멍 | 왜 지금은 못 막나 |
|---|---|
| **사칭** | `?as=`를 남의 아이디로 고치면 그대로 그 사람이 된다. 비밀번호 확인은 로그인 순간 딱 한 번이고, 그 뒤 요청들은 "내가 누구라고 주장하는 값"을 그냥 믿는다. hidden 필드로 숨겨도 개발자도구로 고칠 수 있으니 마찬가지 |
| **CSRF** | 남의 사이트가 우리 폼을 흉내 내 POST를 쏘는 공격. 막으려면 서버가 "내가 발급한 토큰이 맞나"를 대조해야 하는데, **발급한 사실 자체를 기억할 곳이 없다** |

→ 둘 다 **세션이 있어야 풀린다**. week16의 첫 두 항목인 이유다.
...
**세션은 하나도 쓰지 않는다** (week15에서 물려받은 현재 상태 — 11절에서 바꾼다).
서버가 기억해야 할 것은 전부 주소에 실어 나른다 —
