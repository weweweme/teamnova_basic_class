...
- `week16/` : **지금 여기**. week15를 통째로 복사해서 시작 (2026-08-03).
  ...
  - **구현 항목** ①~⑦은 처음 계획, ⑧~⑪은 "쿠키·세션으로 보여줄 게 더 없나"를 파다 추가한 것
    ...
    12. ✅ **글쓰기 임시저장 — 완료.** 새 파일 `includes/drafts.php` + `api/draft.php` + 새 표 `drafts`.
       - 폼에서 입력이 멈추면 **2초 뒤 자동 저장**(디바운스), 페이지를 떠나기 직전엔
         `pagehide` + `navigator.sendBeacon`으로 한 번 더 — 그래서 **새로고침해도 안 날아간다.**
         (떠나는 중에 보낸 `fetch`는 브라우저가 취소해 버린다. sendBeacon은 끝까지 보내준다)
       - `main.js`의 **이탈 경고(`beforeunload`)를 지웠다** — 이제 저장되므로 **거짓말이 되기 때문.**
         막는 장치 → 되살리는 장치로 바꾼 셈. 한계("창을 닫으면 사라집니다")는 화면에 글로 적는다.
  ...
  - **①~⑱ 전부 완료.** ①~⑦은 처음 계획, ⑧~⑱은 "쿠키·세션으로 보여줄 게 더 없나"를 파며 붙인 것.
  - **그릇을 고르는 질문 — 이번 주차의 결론**:
    ...
    - **세션 8개**: `user_id` · `flash` · `csrf_token` · `viewed_posts` · `intended` · `old_input`
      · `auth_at`(sudo) · `last_seen`(유휴) — 대부분 필요할 때 생겼다 쓰면 사라진다.
    - **우리가 심는 쿠키 5개**: `remember` · `pref_sort` · `pref_sentiment` · `recent_search`
      · `recent_posts` (+ PHP가 심는 `PHPSESSID`).
  - **DB 표 11개**가 됐다: week15의 8개 + `remember_tokens`(자동 로그인) + `sessions`(세션 저장소)
    + `drafts`(글 초안). 마이그레이션은 `006_drafts.sql`까지 (001~003 week15, 004~006 week16).
