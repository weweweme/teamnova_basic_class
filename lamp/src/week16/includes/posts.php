...
require_once __DIR__ . '/view_session.php';  // 조회 판정 전용 세션
...
// ── 판정은 세션, 집계는 표 ───────────────────────────────────
//   [★ 왜 판정을 세션으로 되돌렸나]
//     한때 `post_views` 표에 (글, 사람, 날짜)를 남겨 판정했다. 정확했지만 지적을 받았다:
//       *"조회수 뻥튀기를 막는 기술에서, 블랙리스트를 만드는 기술로 넘어갔다."*
//     판정에 필요한 건 **"오늘 이 사람이 이 글을 봤나"** 하나뿐인데,
//     그걸 알자고 **누가 무엇을 봤는지를 서버에 쌓고** 있었다.
//     ★★ 우리 스스로와도 어긋났다 — 쿠키 동의에서 *"최근 본 글은 물어봐야 한다"* 고 해놓고,
//       서버에는 동의 없이 같은 성격의 기록을 남기고 있었다.
//
//   [★ 그래서 판정 기록은 '그 사람 것' 안에 둔다]
//     세션은 **그 방문에 딸린 서랍**이라, 방문이 끝나면 기록도 함께 사라진다.
//     서버 어디에도 *"누가 무엇을 봤다"* 가 모이지 않는다.
//     ※ 우리 세션은 sessions 표에 저장되지만, **그 사람 세션 안**이라는 성질은 같다.
//       (PHP 기본값은 램이 아니라 /tmp의 파일이다. 우리는 그 자리를 DB로 바꿨을 뿐)
//
//   [★★ 그런데 예전 실패를 반복하지 않으려면 두 가지가 필요했다]
//     ① **시각을 함께 담는다.** 예전엔 글 번호만 담아서
//        *"몇 시간 뒤에 다시 셀까"* 를 정할 방법 자체가 없었다.
//        → 이제 [글번호 => 본 날짜] 로 담는다. 하루가 지나면 다시 센다
//     ② **개수에 상한을 둔다.** 예전엔 상한이 없어 payload가 계속 커졌다
//
//     ③ **브라우저가 지울 수 없는 값에서 서랍 번호를 뽑는다.**
//        쿠키에서 뽑으면 **그 쿠키만 지우고 얼마든지 부풀릴 수 있다** — 실제로 그랬다.
//        → 로그인했으면 회원 번호, 아니면 **접속지(IP)**. 자세한 것은 view_session.php.
//
//   [남긴 한계 — 먼저 말할 것]
//     접속지로 판정하므로 **같은 공유기 뒤(집·회사·학교)는 통째로 한 명**이 되고,
//     모바일은 접속지가 자주 바뀌어 **같은 사람이 여러 명**이 된다.
//     ★ 부풀리기를 막는 것과 정확히 세는 것을 저울에 올려, **막는 쪽을 골랐다.**
//     (Discourse도 로그인은 회원 번호, 아니면 IP로 같은 선택을 했다)
// 집계용 — '지금 뜨는 글'이 "최근 며칠"을 보는가.
//   ★ 이 표에는 **누가 봤는지가 없다.** 글별 숫자뿐이라 개인 기록이 아니다.
...
//
//   [왜 이제 가능한가]
//     `post_view_daily`에 **날짜별로 숫자가 남기 때문**이다.
//     ★ 판정(세션)과 집계(표)를 **아예 다른 곳에** 두었다 —
//       집계에는 *누가* 가 없고 **글별 숫자뿐**이라 개인 기록이 아니다.
function recent_view_counts(int $days = TRENDING_DAYS): array {
    // ★ 판정 기록(세션)이 아니라 post_view_daily(몇 번 봤나)에서 읽는다.
    //   판정 기록은 그 사람 세션 안에 흩어져 있어 셀 수가 없고, 셀 필요도 없다.
    $stmt = db()->prepare(
        'SELECT post_id, SUM(views) AS hits
...
// 이 글의 조회수를 올릴까? 올렸으면 true.
//   ★ 판정 기준은 **"오늘 이 세션에서 이 글을 봤나"** 하나다.
function count_post_view(int $id, string $author): bool {
    ...
    //   조회수는 **"남들이 얼마나 봤나"** 인데, 자기가 쓴 글을 확인한 건 그게 아니다.
    //   ⚠️ 그리고 이 숫자는 홈 '지금 뜨는 글'·게시판 조회/인기 탭·랭킹을 움직인다.
    //     하루 1회씩이라도 글쓴이들이 매일 자기 글을 열면 그게 그대로 순위가 된다.
    if (is_owner($author)) {
        return false;
    }
    ...
    db()->prepare(
        'INSERT INTO post_view_daily (post_id, viewed_on, views) VALUES (?, ?, 1)
         ON DUPLICATE KEY UPDATE views = views + 1'
    )->execute([$id, $today]);

    // views = views + 1 을 DB가 직접 계산하게 맡긴다.
    //   ★ PHP로 "읽어서 +1 해서 쓰기"를 하면, 두 사람이 동시에 열었을 때
    //     둘 다 같은 값을 읽고 같은 값을 써서 한 번이 사라진다(경쟁 상태).
    db()->prepare('UPDATE posts SET views = views + 1 WHERE id = ?')->execute([$id]);

    // 창을 넘긴 집계는 버린다. **새로 셌을 때만** 돌아서 부담이 없다.
    db()->prepare('DELETE FROM post_view_daily WHERE viewed_on < ?')
        ->execute([date('Y-m-d', strtotime('-' . TRENDING_DAYS . ' days'))]);
