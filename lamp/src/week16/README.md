...
- **'최근 본 글'은 아직 없다** — week15가 "주소로는 못 나른다"며 미뤄둔 자리.
  이제 담을 금고가 생겼다 → 11절 ④.
...
week15가 **못 만들어서 미뤄둔** 기능(`includes/posts.php` 하단 주석 참고).
`?recent=1,5,9,…`는 볼수록 길어져서 주소로는 나를 수 없었다.

- `$_SESSION`에 글 번호를 앞에서부터 밀어 넣고 최대 N개만 유지.
- 함수 이름은 주석이 예고한 그대로 — `remember_recent_post()` / `get_recent_posts()`.
- **왜 DB가 아니라 세션인가**: 로그인 안 한 사람도 쓰는 기능이라서다.
  DB 표로 만들면 기록의 주인이 '브라우저'에서 '회원'으로 바뀌어 성격이 달라진다.
