...
//
// ★ 이 프로젝트에는 세션이 하나도 없다 (session_start() 없음).
//   세션 = 서버가 '이 브라우저 = 이 사용자'를 기억하는 저장공간이다.
//     로그인하면 서버 금고에 "이 사람 로그인함"을 적어두고,
//     브라우저엔 그 금고의 번호표(쿠키 PHPSESSID)를 준다.
//   지금은 그 금고를 안 쓰기로 하고, 서버가 기억해야 할 것들을 전부 주소에 실어 나른다:
//     · 누구인지   → ?as=영화광        (아래 '신원 이어붙이기' 절)
//     · 알림       → ?flash=…&ftype=…  (아래 '플래시' 절)
//     · 최근 본 글 → 주소로는 못 나른다 → 기능을 week16으로 미뤘다 (posts.php 참고)
